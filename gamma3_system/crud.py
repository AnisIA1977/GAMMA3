...
def create_material_classes_bulk(db: Session, material_classes: list[schemas.MaterialClassCreate]):
    db_objs = [models.MaterialClass(code=cls.code, designation=cls.designation) for cls in material_classes]
    db.add_all(db_objs)
    db.commit()
    for obj in db_objs:
        db.refresh(obj)
...
def create_constructors_bulk(db: Session, constructors: list[schemas.ConstructorCreate]):
    db_objs = [models.Constructor(code=cons.code, designation=cons.designation) for cons in constructors]
    db.add_all(db_objs)
    db.commit()
    for obj in db_objs:
        db.refresh(obj)
...
def create_series_bulk(db: Session, series: list[schemas.SeriesCreate], constructor_id: int):
    db_objs = [models.Series(**ser.model_dump(), constructor_id=constructor_id) for ser in series]
    db.add_all(db_objs)
    db.commit()
    for obj in db_objs:
        db.refresh(obj)
